...
freq_info* build_huffman_tree(tree_queue &queue) {
  ...
  // check for special case: only one elem in queue
  if (queue.size() == 1) {
    root = queue.top();
  }
...
void _build_encoding_map(freq_info* huffman_root, map<char, string> &enc_map, string bs) {
  if (huffman_root) {
    if (huffman_root->is_leaf) {
      char symbol = huffman_root->symbol;
      enc_map.insert( pair<char, string>(symbol, bs) );
    }
    else {
      _build_encoding_map(huffman_root->left, enc_map, bs + LEFT_CHAR);
      _build_encoding_map(huffman_root->right, enc_map, bs + RIGHT_CHAR);
    }
  }
}
...
map<char, string> build_encoding_map(freq_info*& huffman_root) {
  ...
  _build_encoding_map(huffman_root, ret, bs);
...
string encode(string& original, map<char, string>& encoding_map) {
  ...
  string::iterator orig_curr;
  for (orig_curr = original.begin(); orig_curr != original.end(); orig_curr++) {
    string c_enc = encoding_map[*orig_curr];
    result.append(c_enc);
  }
...
string decode(string& encoded, freq_info*& huffman_root) {
  ...
  string::iterator enc_curr;
  for (enc_curr = encoded.begin(); enc_curr != encoded.end(); enc_curr++) {
    if      (*enc_curr == LEFT_CHAR)  cursor = cursor->left;
    else if (*enc_curr == RIGHT_CHAR) cursor = cursor->right;
